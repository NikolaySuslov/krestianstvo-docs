...
import { createLocalStore, collectParentSelos, createLinkForSelo, getSeloByID, Selo, getSelos } from 'krestianstvo'
...
export default function SeloPortal(props) {
	...
	const createFromURL = (data) => {
		...
		props.selo.callAction(props.nodeID, "createSelo", [{
			id: k ? k : undefined,
			reflectorHost: r ? r : props.reflectorHost,
					...
					<Match when={(!local.data.properties.editURL && local.data.dynamicSelo.length == 0)
						...
						<div>
							...
							<input
								...
								placeholder="enter url or world name"
							...
							<div class="flex gap-1 pt1">
								<Switch>
									<Match when={(local.data.properties.url.length > 0 && !local.data.dynamicSelo[0])}>
										<VButton color="green" onClick={[handleClick, ["createPortal"]]}>Create portal</VButton>
									</Match>
									<Match when={(local.data.properties.url.length > 0 && local.data.dynamicSelo[0])}>
									<VButton color="green"  onClick={[handleClick, ["createPortal"]]}>Update portal</VButton>
									</Match>
								</Switch>
								<Show when={local.data.dynamicSelo[0]}>
								<VButton color="green" onClick={[handleClick, ["deleteSelo", local.data.dynamicSelo[0]]]}>Delete portal</VButton>
								</Show>
							</div>
						</div>
					</Match>
